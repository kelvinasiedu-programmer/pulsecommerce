...
1. **Revenue is growing** but **conversion is uneven** across device Ã— channel - mobile Ã— paid-social is the single biggest leak.
...
- Trailing 28-day revenue is up vs the prior 28 days, driven primarily by **Organic Search** and **Email** (see Business Health â†’ Channel mix for the current split).
...
- The **biggest stage drop-off** is **checkout_start â†’ purchase** (see Funnel page for the current conversion rate per stage).
- **Worst segment**: mobile Ã— paid-social, materially below desktop Ã— email conversion.
...
- Churn model **ROC-AUC â‰ˆ 0.889** on the holdout - reliable enough to act on.
...
- Results are sensitive to the next two weeks of cancel-rate behavior.
